...
sed -e "s/replace-with-server-name/${INTEGRATION_SERVER_NAME}/" -e "s~replace-with-namespace~${DEPLOYMENT_NAMESPACE}~" -e "s/replace-with-license/${SERVER_LICENSE}/"  -e "s/replace-with-usage/${SERVER_USAGE}/"  -e "s/replace-with-cpu-limit/${SERVER_CPU_LIMIT}/"  -e "s/replace-with-memory-limit/${SERVER_MEMORY_LIMIT}/"  -e "s/replace-with-cpu-request/${SERVER_CPU_REQUEST}/"  -e "s/replace-with-memory-request/${SERVER_MEMORY_REQUEST}/" -e "s/replace-with-default-app-name/${DEFAULT_APP_NAME}/" -e "s/replace-with-is-version/${INT_SERVER_VERSION}/" -e "s~replace-With-Bar-URL~${BARurl}~" ${CRs_template_folder}/integrationServer.yaml > ${CRs_generated_folder}/integrationServer-generated.yaml
